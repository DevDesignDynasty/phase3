    ...
    const onClickNext = () => {
        ...
        if (isSubmitted) {
            ...
            if (currentQuestion !== questions.length - 1) {
                ...
                submitQuizResults(result.score);
                setShowResults(true);
            }
        }
    };

    //save quiz marks in the backend
    const submitQuizResults = async (score) => {
        const data = score; // Data to be sent in the PUT request
    
        try {
            const res = await axios.put(`https://backendgame.sytes.net/submit/${token}`, data, {
                headers: {
